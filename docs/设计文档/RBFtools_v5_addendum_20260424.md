...
| **B2** | 多源驱动异构 | ⚠️ 高 | **⚠️ C++ `driverList` 已 multi 但 UI/core 单 driver** | —— | **M_B24** (a/b) |
| B3 | 输入编码枚举 5 档 | ❌ 致命 | **✅ 完整** | M2.1a (`8541d4d`-pre) / M2.1b | 已完成 |
| **B4** | 输入 Quat / 输出 Euler 分离 | ❌ 高 | **❌ 无 `outputEncoding` MObject** | —— | **M_B24** (合并) |
...
class TestV5ParityComplete(unittest.TestCase):
    ...
    def test_b2_multi_source_driver(self):
        """M_B24a deliverable: driverSource compound multi
        attribute exists in C++ schema; core.py exposes
        add_driver_source / remove_driver_source / read_driver_info_multi;
        compute() iterates driverList[] for aggregate input vector;
        legacy single-driver auto-migrates with cmds.warning."""
        # Enabled by M_B24a commit.

    def test_b4_output_encoding(self):
        """M_B24a deliverable: outputEncoding MObject enum exists
        with values 0=Euler, 1=Quaternion, 2=ExpMap; compute()
        applies the inverse encoding to output[]."""
        # Enabled by M_B24a commit (bundled with B2).
