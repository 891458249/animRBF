# -*- coding: utf-8 -*-
u"""Auto-neutral-sample seeding for fresh RBFtools nodes (Milestone 3.6).
...
def generate_neutral_values(n_outputs, output_is_scale=None,
    ...
    u"""Build the rest-pose driven-values vector.
...
def add_neutral_sample(node):
    u"""Append a single rest pose to *node* at index 0.
    ...
    # ---- Read current shape state (single code path, contract ??) ----
    ...
    #      Apply step's responsibility -- see addendum sec.M3.6 Q8).
    with core.undo_chunk("RBFtools: add neutral sample"):
        ...
        if existing:
            ...
            shifted = [
                # Commit 1: index shift preserves per-pose sigma.
