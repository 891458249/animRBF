...
class _PoseEditorPanel(CollapsibleFrame):
    u"""Composes the driver/driven attribute lists, the QTableView and
    ...
    def add_pose_row_action(self, label_key, callback, danger=False):
        u"""M3.0-spillover (added in M3.2 commit per addendum \u00a7M3.0.5):
...
class StatusProgressController(object):
    u"""Encapsulates the 3-state status-bar progress lifecycle (M3.0).
...
class RBFToolsWindow(QtWidgets.QMainWindow):
    ...
    def _build_ui(self):
        ...
        # "??????UI??"): the standalone Driver Sources +
        ...
        # sec.M3.0 access path A -- the recommended MVC route).
        ...
        # ---- M3.3: File menu entries via M3.0-spillover sec.2 ----
        ...
        # (M3.0-spillover sec.3 lazy creation happens inside the
        ...
        # ---- M3.4: Live Edit Mode toggle (spillover sec.3 second consumer)
    ...
    def _build_menu_bar(self):
        u"""Create the top-level menu bar (M3.0 baseline + M3.x add-ons).
        ...
        # constitutes user intent (addendum sec.M3.0 (G)?).
    ...
    # =================================================================
    #  M3.5 -- Pose Profiler entry + ToolsSection spillover sec.3
    # =================================================================
    ...
    def _ensure_tools_section(self):
        u"""Lazily create the per-node ToolsSection collapsible.
    ...
    def add_tools_panel_widget(self, widget_id, widget):
        u"""Register *widget* under *widget_id* in the per-node
    ...
    def remove_tools_panel_widget(self, widget_id):
        u"""Unregister and detach a previously-added Tools panel
    ...
    def _on_remove_pose_row(self, row_idx):
        u"""Right-click pose-table -> Remove this pose (M3.1 single
    ...
    def _on_export_poses(self):
        u"""File -> Export Poses Only... (M_P0_POSES_IO)
    ...
    def _on_import_poses(self):
        u"""File -> Import Poses Only... (M_P0_POSES_IO)
    ...
    def add_file_action(self, label_key, callback):
        u"""M3.0-spillover \u00a72 (added in M3.3 commit per addendum
    ...
    def _on_mirror_pose(self, row_idx):
        u"""Right-click pose-table -> Mirror this pose (M3.2 single-pose
        ...
        _cmds.warning(
            ...
            u"M3.2 patch \u2014 please use Tools -> Mirror Node... for "
            "now.".format(row_idx))
    ...
    def _connect_signals(self):
        ...
        # sec.M_UIRECONCILE.m_b24b1-correction). The widget emits
    ...
    def _on_nodes_refreshed(self, names):
        u"""M_P0_REFRESH_KEEPS_CURRENT (2026-04-30): the upstream
    ...
    def _on_settings_loaded(self, data):
        ...
        # "?? RBF" while shape.rbfMode was 0, and driver->driven
    ...
    def _on_driven_source_attrs_apply(self, index, attrs):
        u"""M_CONNECT_DISCONNECT_FIX Bug 1 driven mirror \u2014 same
    ...
    def _on_driven_source_attrs_clear(self, index, attrs):
        u"""M_CONNECT_DISCONNECT_FIX Bug 2 driven mirror \u2014 Scene
    ...
    def _on_pose_grid_update(self, pose_index):
        u"""M_P0_UPDATE_BUTTON_REVERSED (2026-04-30): per-row Update
    ...
    def _on_base_pose_value_changed(self, _pose_idx, side,
                                    source_idx, attr_name, value):
        u"""Commit 3 (M_BASE_POSE): BasePose driven-side spinbox edit.
    ...
    def _on_driver_source_attrs_clear(self, index, attrs):
        u"""M_CONNECT_DISCONNECT_FIX Bug 2 (2026-04-28): Disconnect
    ...
    def _guard_attrs_apply(self, role, index, attrs):
        u"""M_CONNECT_DISCONNECT_FIX \u52a0\u56fa 2 (2026-04-28): plan-dict
    ...
    def _on_editor_loaded(self):
        ...
        # reported "???? pose ????" P0. Empty-node path
    ...
    def _gather_routed_targets(self):
        u"""2026-04-28 (M_BATCH_ROUTING + M_CRASH_FIX defense 1 +
        ...
        # / ``a`` instead of ``str(...)``. Under py2, ``str(u"??")``
...
def show():
    u"""Show the RBF Tools window (**singleton** pattern).
