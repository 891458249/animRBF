# -*- coding: utf-8 -*-
u"""Mirror math + naming-rule helpers for the Mirror Tool (M3.2).
...
def mirror_quaternion(q, axis):
    u"""Mirror a unit quaternion (qx, qy, qz, qw) across a coord plane.
...
def mirror_raw_attr_value(attr_name, value, axis):
    u"""Mirror a single Maya transform-attr value.
...
def apply_naming_rule(name, rule_index, custom=None, direction="auto"):
    u"""Apply a naming rule to *name* and return ``(new_name, status)``.
...
def mirror_driver_inputs(inputs, encoding, axis, driver_attrs=None):
    u"""Mirror a flat list of driver-input values.
    ...
    if encoding == _ENC_BENDROLL:
        ...
        # scope per addendum sec.M3.2 (E)?. Pass-through with flag.
