# -*- coding: utf-8 -*-
u"""JSON I/O utilities for v5 RBFtools schema (Milestone 3.0).
...
# === Schema version (addendum sec.M3.0 + sec.M_B24a2 -- bump protocol) ===
...
# sec.M_B24a2 PROJECT-CONSTITUTIONAL-EVENT for the precedent.
...
LEGACY_SCHEMA_VERSIONS = frozenset({
    "rbftools.v5.m3",
    "rbftools.v5.m_b24",  # Commit 1: per-pose sigma + base pose are
...
def atomic_write_json(path, data):
    u"""Write *data* to *path* atomically.
...
#       capture path. THIS IS THE ONLY LEGAL BYPASS (addendum sec.M3.3.G).
...
# Locked by SCHEMA_VERSION; see addendum sec.M3.3.1.
...
def _upgrade_legacy_node(ndata):
    u"""In-memory upgrade of a single legacy v5.0-pre-M_B24 node dict
...
def _upgrade_legacy_dict(data):
    u"""In-memory upgrade of a top-level v5.0-pre-M_B24 document dict
...
def node_to_dict(node):
    u"""Read a fully-loaded RBFtools node into a JSON-ready dict.
...
def _attr_default(maya_attr):
    u"""Return the export-time default for *maya_attr* \u2014 used only as
...
def export_nodes_to_path(node_names, path, meta=None):
    u"""Bake *node_names* (list of transform names) into a single JSON
...
def _validate_node_dict(ndata, rpt, mode, idx):
    u"""Per-node validation \u2014 collects errors into *rpt* (F.3).
...
def _validate_attr_array(arr, prefix, scene_node, rpt):
    u"""Validate sparse-multi-aware attr array (red line #5).
...
def import_path(path, mode="add"):
    u"""Two-phase import (E.2): dry-run all nodes first, then execute
...
def dict_to_node(ndata, mode="add", will_overwrite=False):
    u"""Materialise one node-dict into the scene.
...
def export_poses_to_path(node, path):
    u"""Export *node*'s pose data + driver/driven signature to *path*.
...
def _validate_poses_payload(data, target_node):
    ...
    if sv != POSES_SCHEMA_VERSION:
        raise PosesImportValidationError(
            u"import_poses: schema_version mismatch \u2014 file has {!r}, "
    ...
    if not isinstance(src_meta, dict) or not isinstance(poses_arr, list):
        raise PosesImportValidationError(
            u"import_poses: malformed file \u2014 missing 'exported_from' "
    ...
    if file_in_count != target_in_count:
        raise PosesImportValidationError(
            u"import_poses: driver INPUT count mismatch \u2014 file has {} "
    ...
    if file_out_count != target_out_count:
        raise PosesImportValidationError(
            u"import_poses: driven OUTPUT count mismatch \u2014 file has {} "
    ...
    if (file_drv.get("node") or "") != (drv_node_target or ""):
        warnings_list.append(
            ...
            u"match \u2014 proceeding with import).".format(
    ...
    if list(file_drv.get("attrs") or []) != list(drv_attrs_target or []):
        warnings_list.append(
            ...
            u"(positional \u2014 counts match, proceeding).".format(
