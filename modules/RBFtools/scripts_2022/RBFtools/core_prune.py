# -*- coding: utf-8 -*-
u"""Pose Pruner \u2014 data-hygiene analysis + execute (Milestone 3.1).
...
class QuatGroupEffect(object):
    u"""One quat-group's after-prune fate.
...
class PruneAction(object):
    ...
    def has_changes(self):
        u"""Return True iff at least one prune class would actually do
...
def _scan_duplicates(poses, abs_tol=_PRUNE_ABS_TOL):
    u"""Scan a list of PoseData for (input AND value) duplicates and
...
def shift_quat_starts(starts, removed_output_indices):
    u"""Compute new quat-group start indices after some output indices
    are removed.
...
def analyse_node(node, opts=None):
    u"""Read-only dry-run analysis (T13 PERMANENT \u2014 no mutation).
...
def execute_prune(node, action):
    u"""Apply *action* to *node* by rebuilding the packed pose data.
    ...
    for new_idx, p in enumerate(keep_pose):
        # Commit 1: prune rebuild preserves per-pose sigma for the surviving
