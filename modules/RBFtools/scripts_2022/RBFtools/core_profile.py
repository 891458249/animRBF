# -*- coding: utf-8 -*-
u"""Pose Profiler \u2014 read-only diagnostic + split suggestion (Milestone 3.5).
...
def _estimate_memory(n_poses, n_inputs, n_outputs):
    u"""Approximate per-node memory footprint in bytes.
...
def profile_node(node):
    u"""Read-only diagnostic snapshot of *node*.
...
def format_report(profile):
    ...
    bar = "=" * 56
    lines.append(bar)
    lines.append(u"RBFtools Profile \u2014 {}".format(p["node_name"]))
    lines.append(bar)
    lines.append("")
    ...
    lines.append("                       evaluation is held in a C++ "
                 "instance member and")
    lines.append(u"                       is not exposed to Python \u2014 "
                 u"see addendum \u00a7M3.5.F1]")
    lines.append("")
    ...
    lines.append("Performance estimates  "
                 u"[CONCEPTUAL \u2014 no machine calibration]")
    ...
    lines.append(u"   for your hardware; see addendum \u00a7M3.5.F2]")
    lines.append("")
    ...
    if triggers:
        ...
        lines.append("         Splitting strategy is rig-semantic "
                     "and must be")
        lines.append(u"         decided by the user \u2014 Profiler does "
                     "not auto-split")
