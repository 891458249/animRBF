# -*- coding: utf-8 -*-
u"""Live Edit Mode \u2014 pure throttle + scriptJob lifecycle (Milestone 3.4).
...
class ThrottleState(object):
    u"""Mutable state for a hybrid leading+trailing throttle.
...
def mark_emitted(state, now_ts):
    u"""Advance the state after the caller has actually fired its
...
def planned_transition_on_node_change(current_state, new_driver_attrs):
    u"""Caller invokes this when the active RBFtools node changes
