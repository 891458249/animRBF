# -*- coding: utf-8 -*-
u"""Auto-alias generation for RBFtools shape input[]/output[] multi plugs
(Milestone 3.7).
...
def generate_alias_name(attr_name, idx, role, is_quat_group_leader=False):
    u"""Build the canonical alias name for one ``input[idx]`` /
    ``output[idx]`` plug.
...
def is_rbftools_managed_alias(name):
    u"""Return True iff *name* looks like an RBFtools-generated alias.
...
def clear_managed_aliases(shape):
    u"""Remove every alias on *shape* that :func:`is_rbftools_managed_alias`
...
def _set_one_alias(shape, alias, plug_path, used_names):
    u"""Apply a single alias with conflict-aware fallback (C.3.a / c).
...
def apply_aliases(shape, driver_attrs, driven_attrs,
    ...
    u"""Apply aliases to *shape*'s ``input[]`` / ``output[]`` plugs.
...
def read_aliases(shape):
    u"""Return ``{"input": {idx: alias}, "output": {idx: alias}}`` \u2014
